    ...
    {    	
        ...
        private void  Send(boolean signal) 
        {
            ...
            try {
                ...
                outputStream.write(_sendFrame.GetL1() | 0x80 | (_sendFrame.GetACKRequired() ? 0x40 : 0x00));

                outputStream.write(_sendFrame.GetCommandID().getValue() | 0x80);

                outputStream.write(_sendPayload.GetData(), 0, _sendPayload.GetPosition());
        ...
        private boolean FrameReceive() throws IOException
        {
            ...
            while (_receivePayload.GetPosition() < _size)
                _receivePayload.SetPosition(_receivePayload.GetPosition() + inputStream.read(_receivePayload.GetData(), _receivePayload.GetPosition(), _size - _receivePayload.GetPosition()));

            _receiveFrame.SetChecksum(inputStream.read());

            if (inputStream.read() == UARTFrame.EOM && _receiveFrame.VerifyChecksum())
                return true;
            else return false;
        }
        	...
        	public void run(){
                ...
                while (true)
                {
                    try
                    {
                        ...
                        if (isSignal && _receiveFrame.GetCommandID() == _sendFrame.GetCommandID() && _receivePayload.GetSubCommandID() == _sendPayload.GetSubCommandID() && _receivePayload.GetFrameID() == _sendPayload.GetFrameID())
                        {
                            isSignal = false;
                            _safePayload.Rewind();
                ...
                case 0x00 :// SubCommandID.GEN_PWR_UP_IND.getValue()
                    if (listeners != null)
                    	for (MuRataIndicationListener listener : listeners)
                    		listener.onPowerUpIndication(new PowerUpIndication(_receivePayload));
                    break;
            }
        }
        ...
        private void WIFIIndication()
        {
            switch (_receivePayload.GetSubCommandID())
            {
                case 0x11 : // SubCommandID.WIFI_SCAN_RESULT_IND.getValue()
                    if (listeners != null)
                    	for (MuRataIndicationListener listener : listeners)
                    		listener.onScanResultIndication(new SSIDRecordIndication(_receivePayload));
                    break;

                case 0x10 : // SubCommandID.WIFI_NETWORK_STATUS_IND.getValue()
                    if (listeners != null)
                    	for (MuRataIndicationListener listener : listeners)
                    		listener.onWIFIConnectionIndication(new WIFIConnectionIndication(_receivePayload));
                    break;
            }
        }
        ...
        private void SNICIndication()
        {
            switch (_receivePayload.GetSubCommandID())
            {
                case 0x20 : // SubCommandID.SNIC_TCP_CONNECTION_STATUS_IND.getValue()
                    if (listeners != null)
                    	for (MuRataIndicationListener listener : listeners)
                    		listener.onTcpConnectionStatusIndication(new TCPStatusIndication(_receivePayload));
                    break;
                case 0x22 : // SubCommandID.SNIC_CONNECTION_RECV_IND.getValue()
                    if (listeners != null)
                    	for (MuRataIndicationListener listener : listeners)
                    		listener.onSocketReceiveIndication(new SocketReceiveInidcation(_receivePayload));
                    break;
                case 0x23 : // SubCommandID.SNIC_UDP_RECV_IND.getValue()
                    if (listeners != null)
                    	for (MuRataIndicationListener listener : listeners)
                    		listener.onUDPReceiveIndication(new UDPReceivedIndication(_receivePayload));
                    break;
                case 0x25 : // SubCommandID.SNIC_HTTP_RSP_IND.getValue()
                    if (listeners != null)
                    	for (MuRataIndicationListener listener : listeners)
                    		listener.onHTTPResponseIndication(new HTTPResponseIndication(_receivePayload));
                    break;
            }
        }
...
//        }
        ...
        /// <returns></returns>
        public WIFICode WIFI_ScanNetworks(ScanType scan, BSSType bss, byte[] BSSID, byte[] channelList, String SSID) throws Exception
        {
